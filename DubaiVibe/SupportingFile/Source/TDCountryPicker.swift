...
struct Section {
    var countries: [TDCountry] = []
    mutating func addCountry(_ country: TDCountry) {
        countries.append(country)
    }
}

@objc public protocol TDCountryPickerDelegate: class {
    @objc optional func countryPicker(_ picker: TDCountryPicker,
                       didSelectCountryWithName name: String,
                       code: String)
    func countryPicker(_ picker: TDCountryPicker,
                                      didSelectCountryWithName name: String,
                                      code: String,
                                      dialCode: String)
}

open class TDCountryPicker: UITableViewController {
    
    private var customCountriesCode: [String]?
    
    lazy var CallingCodes = { () -> [[String: String]] in
        let resourceBundle = Bundle(for: TDCountryPicker.classForCoder())
        guard let path = resourceBundle.path(forResource: "CallingCodes", ofType: "plist") else { return [] }
        return NSArray(contentsOfFile: path) as! [[String: String]]
    }()
    fileprivate var searchController: UISearchController!
    fileprivate var filteredList = [TDCountry]()
    fileprivate var unsortedCountries : [TDCountry] {
        let locale = Locale.current
        var unsortedCountries = [TDCountry]()
        ...
        for countryCode in countriesCodes {
            ...
            let country: TDCountry
            
            if countryData.count > 0, let dialCode = countryData[0]["dial_code"] {
                country = TDCountry(name: displayName!, code: countryCode, dialCode: dialCode)
            } else {
                country = TDCountry(name: displayName!, code: countryCode)
            }
    ...
    fileprivate var sections: [Section] {
        ...
        let countries: [TDCountry] = unsortedCountries.map { country in
            let country = TDCountry(name: country.name, code: country.code, dialCode: country.dialCode)
            country.section = collation.section(for: country, collationStringSelector: #selector(getter: TDCountry.name))
            return country
        }
        ...
        for section in sections {
            var s = section
            s.countries = collation.sortedArray(from: section.countries, collationStringSelector: #selector(getter: TDCountry.name)) as! [TDCountry]
        }
    ...
    open weak var delegate: TDCountryPickerDelegate?
    ...
    fileprivate func filter(_ searchText: String) -> [TDCountry] {
        filteredList.removeAll()
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !query.isEmpty else { return filteredList }

        sections.forEach { section in
            section.countries.forEach { country in
                if country.matches(searchText: query) {
    ...
    fileprivate func getCountry(_ code: String) -> [TDCountry] {
        filteredList.removeAll()
    ...
    public func getFlag(countryCode: String) -> UIImage? {
        ...
        if countries.count == 1 {
            ...
            return UIImage(named: bundle + countries.first!.code.uppercased() + ".png",
                           in: Bundle(for: TDCountryPicker.self), compatibleWith: nil)
        }
        else {
            return nil
        }
...
extension TDCountryPicker {
    ...
    override open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if showCallingCodes {
            cell.textLabel?.text = country.displayTitle + " (" + country.dialCode! + ")"
        } else {
            cell.textLabel?.text = country.displayTitle
        }
        
        let bundle = "assets.bundle/"
        
        if self.showFlags == true {
            let image = UIImage(named: bundle + country.code.uppercased() + ".png", in: Bundle(for: TDCountryPicker.self), compatibleWith: nil)
            if (image != nil) {
...
extension TDCountryPicker: UISearchResultsUpdating {
